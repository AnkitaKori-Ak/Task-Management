...
useEffect(() => {
  const fetchTasks = async () => {
    try {
      const res = await axios.get(`http://localhost:3000/api/task/getUserTasks?userId=${userId}`,{ withCredentials: true });
    ...
    setTasks((prevTasks) =>
      prevTasks.map((task) => {
        if (task.n_tid === taskId) {
          ...
          // Optimistically update state with new description
